...
{    
    for(int i = 0; i < numRooms; i++)
    {
        ...
        ROOM&& createdRoom = CreateRoom(rooms, ROOM::RECTANGULAR, maxRoomSize, levelSize, levelSize);
        ...
        if(createdRoom.w > 0)
        {
            ...
            {
                bool whichSide = ScionEngine::GetRandomNumber(0, 1) > 0; //false = top/left, true = bot/right
                bool topOrLeft = ScionEngine::GetRandomNumber(0, 1) > 0; //false = horizontal, true = vertical
                ...
                {            
                    whichSide = ScionEngine::GetRandomNumber(0, 1) > 0; //false = top/left, true = bot/right
                    topOrLeft = ScionEngine::GetRandomNumber(0, 1) > 0; //false = horizontal, true = vertical
            ...
            for(size_t i = 0; i < roomTiles.size(); i++)
...
void LevelGenerator::GenerateCorridors(Level& level)
{
    ...
    //for(size_t i = ScionEngine::GetRandomNumber(1, level.GetWidth()); i < level.GetWidth(); i = ScionEngine::GetRandomNumber(1, level.GetWidth()))
    for(size_t i = 1; i < level.GetWidth()-1; i++)
    {
        //for(size_t j = ScionEngine::GetRandomNumber(1, level.GetHeight()); j < level.GetHeight(); j = ScionEngine::GetRandomNumber(1, level.GetHeight()))
        for(size_t j = 1; j < level.GetHeight()-1; j++)
    //for(auto itr = begin(roomMap), itrEnd = end(roomMap); itr != itrEnd; itr++)
        {
        //auto room = itr->second.get();

        //for(size_t t = 0; t < room->tiles.size(); t++)
        //{
...
{
    for(size_t i = 0; i < corridorSeeds.size(); i++)
...
bool LevelGenerator::CheckBranchDown(Level& level, Branch* branch)
{
    ...
    //    
    //    for(size_t j = 0; j < branch->tiles.size(); j++)
    ...
    for(size_t i = 0; i < branch->children.size(); i++)
    {
        ...
        {
            //if(c)
                for(size_t j = 0; j < c->tiles.size(); j++)
...
{
    for(size_t i = 0; i < level.GetWidth(); i++)
        for(size_t j = 0; j < level.GetHeight(); j++)
    ...
    q.insert(tile);
    Tile* n =  nullptr, *w,*e, *nextW, *nextE, *tile2;
...
Level LevelGenerator::CreateLevelWithRooms1(Level::SIZE levelSize, ROOM::SIZE maxRoomSize)
{
    ...
    //generate corridors
    //for(size_t x = 1; x < level.GetWidth(); x+=2)
    //    for(size_t y = 1; y < level.GetWidth        (); y+=2)
    //    {
        ...
        for(size_t j = i+1; j < rooms.size(); j++)
        {
            ...
            if(rooms[i] != rooms[j] && !checked)
            {
                ...
                if(path.size() == 0)
                {
                    ...
                    for(size_t i = 0; i < nearestTilePath.size(); i++)
...
Level LevelGenerator::CreateLevelWithBSP(Level::SIZE levelSize, ROOM::SIZE maxRoomSize)
{
    ...
    auto getRandomSplitNumber = [](double sL, double sH, bool hSplit, LEVEL_PARTITION& sdung) -> int
    {
        return ScionEngine::GetRandomNumber(
                (!hSplit*(sdung.x + (int)(sdung.w*sL))) + (hSplit*(sdung.y + (int)(sdung.h*sL))),
                (!hSplit*(sdung.x + (int)(sdung.w*sH))) + (hSplit*(sdung.y + (int)(sdung.h*sH))));
    };
...
tsanhs:
    ...
    for(size_t i = 0; i < subdungeons.size(); i++)
    {
        ...
        {
            rooms.pop_back();
            createdRoom = CreateRoom(rooms, ROOM::RECTANGULAR, sf::IntRect(d.x, d.y, d.w, d.h));
